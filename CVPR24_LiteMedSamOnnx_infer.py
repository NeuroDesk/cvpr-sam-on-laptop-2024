...
import onnxruntime
from src.infer_util import medsam_preprocess
#%% set seeds
...
def get_bbox_from_mask(mask_256, bbox_shift=4):
    """
...
def MedSAM_infer_npz_2D(img_npz_file, encoder_session, decoder_session, image_size):
    ...
    ## preprocessing
    img_tensor, new_H, new_W  = medsam_preprocess(img_3c, image_size)
    ...
    for idx, box in enumerate(boxes, start=1):
        ...
        medsam_mask, iou_pred = onnx_decoder_inference(decoder_session, image_embedding, [], box_resized, (new_H, new_W), [H, W])
...
def MedSAM_infer_npz_3D(img_npz_file, encoder_session, decoder_session, image_size, view):
    ...
    for idx, box3D in enumerate(boxes_3D, start=1):
        ...
        for i in range(i_middle, i_max):
            ...
            H, W = img_2d.shape
            img_tensor, new_H, new_W  = medsam_preprocess(img_2d, image_size)
            ...
            if i == i_middle:
                ...
                if np.max(pre_seg) > 0 and ('MR' in npz_name or 'CT' in npz_name or 'US' in npz_name or 'Endo' in npz_name):
            ...
            img_2d_seg, iou_pred = onnx_decoder_inference(decoder_session, image_embedding, [], box_resized, [new_H, new_W], [H, W])
...
if __name__ == '__main__':
    ...
    for img_npz_file in tqdm(img_npz_files):
        ...
        if basename(img_npz_file).startswith('2D'):
            MedSAM_infer_npz_2D(img_npz_file, encoder_session, decoder_session, image_size)
        elif basename(img_npz_file).startswith('3D') and "PET" in basename(img_npz_file):
            axial = MedSAM_infer_npz_3D(img_npz_file, encoder_session, decoder_session, image_size, 'axial')
            coronal = MedSAM_infer_npz_3D(img_npz_file, encoder_session, decoder_session, image_size, 'coronal')
            sagittal = MedSAM_infer_npz_3D(img_npz_file, encoder_session, decoder_session, image_size, 'sagittal')
